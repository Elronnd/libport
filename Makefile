...
CFLAGS += -Iinclude -I$(HEADER_DIR)/moar -I$(HEADER_DIR)/dyncall -I$(HEADER_DIR)/libtommath -DPERL6_INSTALL_PATH='"$(BUNDLE_DIR)"'
...
test: static $(TEST_OBJ)
	$(CC) $(CFLAGS) $(TEST_OBJ) libport.a -o run-tests -Wl,-rpath=$(BUNDLE_DIR)/lib/ -L$(BUNDLE_DIR)/lib -lmoar
	./run-tests

clean:
	rm -f $(OBJ) $(EXAMPLE_OBJ) $(TEST_OBJ) run-tests libport.a libport.so example-dynamic example-static src/fairy-juice.h
